...
The `TextureAtlas` asset that previously contained both the atlas layout and image handle has been removed, and replaced by:

- A new `TextureAtlasLayout` type.
- A separate `Handle<Image>` (`SpriteSheetBundle::texture` or `AtlasImageBundle::texture`).

`TextureAtlasSprite` was removed and replaced by:

- A new component `TextureAtlas` now holds the atlas index.
- The `Sprite` component can be used for `flip_x`, `flip_y`, `custom_size`, `anchor`, and `color`.
