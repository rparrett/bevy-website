...
Existing scene files containing `Name` components will need to be updated if they are deserialized with Bevy's `serialize` feature enabled (disabled by default).

```plain
// 0.12
"bevy_core::name::Name": (
    hash: 188569425684449923,
    name: "MyComponent",
),
// 0.13
"bevy_core::name::Name": "MyComponent"
```

If you're affected by this breakage, you'll see an error like this:

```plain
Failed to load asset '...' with asset loader 'bevy_scene::scene_loader::SceneLoader': Could not parse RON: 35:34: Expected string
```
